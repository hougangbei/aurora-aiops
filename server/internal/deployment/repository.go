...
func (r *Repository) PutSecret(ctx context.Context, serverID, projectID, kind string, sealed SealedSecret) error {
	...
	_, err := r.db.ExecContext(ctx, deploymentSecretUpsertSQL, id, serverID, projectID, kind, sealed.Nonce, sealed.Ciphertext, sealed.KeyVersion, now, now)
	if err != nil {
...
// SaveManagedInstallation commits the encrypted kubeconfig and its public
// installation summary in one transaction. The summary is deliberately
// caller-provided metadata and never contains the kubeconfig bytes.
func (r *Repository) SaveManagedInstallation(ctx context.Context, serverID, projectID, version, taskID, health string, sealed SealedSecret) error {
	if serverID == "" || projectID == "" || version == "" || taskID == "" || health == "" {
		return ErrInvalidInput
	}
	if err := validateResourceSecret(serverID, projectID, "kubeconfig", sealed); err != nil {
		return err
	}
	...
	defer tx.Rollback()
	now := formatTime(r.now())
	id := serverID + "\x00" + projectID + "\x00kubeconfig"
	if _, err := tx.ExecContext(ctx, deploymentSecretUpsertSQL, id, serverID, projectID, "kubeconfig", sealed.Nonce, sealed.Ciphertext, sealed.KeyVersion, now, now); err != nil {
		return fmt.Errorf("save deployment secret: %w", err)
	}
	installationID := serverID + "\x00" + projectID
	if _, err := tx.ExecContext(ctx, `INSERT INTO project_installations (id,server_id,project_id,version,status,health_summary,last_task_id,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?) ON CONFLICT(server_id,project_id) DO UPDATE SET version=excluded.version,status=excluded.status,health_summary=excluded.health_summary,last_task_id=excluded.last_task_id,updated_at=excluded.updated_at`, installationID, serverID, projectID, version, "managed", health, taskID, now, now); err != nil {
		return fmt.Errorf("save installation summary: %w", err)
	}
...
const deploymentSecretUpsertSQL = `INSERT INTO deployment_secrets (id,server_id,project_id,secret_kind,nonce,ciphertext,key_version,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?) ON CONFLICT(server_id,project_id,secret_kind) DO UPDATE SET nonce=excluded.nonce,ciphertext=excluded.ciphertext,key_version=excluded.key_version,updated_at=excluded.updated_at`
