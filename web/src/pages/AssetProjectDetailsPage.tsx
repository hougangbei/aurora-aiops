...
export function AssetProjectDetailsPage() {
  ...
  return <section className="aurora-panel space-y-5 rounded-[24px] border p-6">
    ...
    <InstallProjectModal open={installOpen} project={project} servers={serversQuery.data ?? []} canInstall={canInstall} disabledReason={dataMode === 'demo' ? '演示模式为只读。' : '当前角色无权安装项目。'} onClose={() => setInstallOpen(false)} />
  </section>;
}
